...
export default function NewsGridClient({ items }: { items: NewsItem[] }) {
  ...
  const filteredItems = items.filter(item => {
    ...
    if (activeTab === 'active') return !item.isMore;
    if (activeTab === 'archive') return item.isMore;
    return true;
  });
    ...
    <div className="w-full">
      ...
      <div className="flex flex-wrap items-center justify-center gap-3 mb-10">
        <button 
          ...
          className={`px-6 py-2.5 rounded-full font-medium transition-all duration-300 ${activeTab === 'all' ? 'bg-blue-500 text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/10'}`}
        >
          Tất cả
        </button>
        <button 
          onClick={() => { setActiveTab('active'); setVisibleCount(12); }}
          className={`px-6 py-2.5 rounded-full font-medium transition-all duration-300 ${activeTab === 'active' ? 'bg-blue-500 text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/10'}`}
        >
          Tin Nổi Bật
        </button>
        <button 
          onClick={() => { setActiveTab('archive'); setVisibleCount(12); }}
          className={`px-6 py-2.5 rounded-full font-medium transition-all duration-300 ${activeTab === 'archive' ? 'bg-blue-500 text-white shadow-[0_0_15px_rgba(59,130,246,0.5)]' : 'bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 border border-white/10'}`}
        >
          Archive (Lưu trữ)
        </button>
      </div>
            ...
            >
              ...
              <div className="p-5 flex flex-col flex-1">
                <div className="flex items-center gap-2 mb-3">
                  <span className="px-2.5 py-1 text-[10px] font-bold tracking-wider uppercase rounded-full bg-blue-500/20 text-blue-300 border border-blue-500/30">
                    News
                  </span>
      ...
      {visibleCount < filteredItems.length && (
        <div className="mt-16 flex justify-center">
          ...
          >
            Tải thêm tin tức ({filteredItems.length - visibleCount})
      ...
      {filteredItems.length === 0 && (
        <div className="text-center py-24 bg-black/20 rounded-3xl border border-white/10 mt-8">
          ...
          <h3 className="text-xl text-gray-400 font-medium tracking-wide">Chưa có tin tức nào</h3>
        </div>
      )}
    </div>
  );
